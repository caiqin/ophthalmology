...
public class DateUtils {
	 ...
	 * @return
	 */
	public static Date getTimeAddHour(Date time,long hour) throws ParseException{
		long startTimes = time.getTime();
		long seconds = hour*60*60*1000;
		Date endTime = new Date(startTimes+seconds); 
		return endTime;
	}
	/**
	 * 获取两个时间的差
	 ...
	 * @return
	 */
	public static double getHours(Date startTime,Date endTime) {
		long from = startTime.getTime();  
		long to = endTime.getTime();  
		BigDecimal bd = new BigDecimal(to-from);
		BigDecimal divide = bd.divide(new BigDecimal(1000 * 60 * 60),2);
		double hours = divide.doubleValue();  
		return hours;
	}
	public static void main(String[] args) throws ParseException {
		Date startTime = DEFAULT_DATETIME_FORMATER.parse("2018-04-18 12:00:00");
